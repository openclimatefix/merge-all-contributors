...
async function getAllContributorsForRepo(repo_handle) {
  const { status, data } = await axios.get(
    `https://raw.githubusercontent.com/${repo_handle}/main/.all-contributorsrc`,
    {
...
async function main(org) {
  ...
  const getAllContributorsPromises = allRepos.map((repo) =>
    getAllContributorsForRepo(repo.full_name),
  );
  Promise.all(getAllContributorsPromises).then((allContributorFiles) => {
    const allContributorsDict = allContributorFiles.reduce(
      (result, contributorFile) => {
          ...
          for (contributor of contributorFile.contributors) {
            if (contributor.profile in result) {
              for (contributionType of contributor.contributions) {
                if (
                  result[contributor.profile].contributions.indexOf(
                    contributionType,
                  ) === -1
                ) {
                  result[contributor.profile].contributions.push(
                    contributionType,
                  );
                }
              }
